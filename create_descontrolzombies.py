...
# Si ya existe una carpeta anterior, la reemplaza
if GAME_DIR.exists():
    shutil.rmtree(GAME_DIR)

# .gitignore (solo lo crea si no existe)
...
func _on_spawn_timer() -> void:
    ...
    if enemies.size() < 4:
        ...
        _spawn_zombie(spawn_positions[index])
  ...
  build-android:
    ...
    steps:
      ...
      - name: Import project (headless)
        shell: bash
        run: |
          godot --headless --path DescontrolZombies --editor --quit-after 10
...
- Movimiento: inclinación (tilt) + botones táctiles
- Animación objetivo: 15 FPS uniforme (cuando integremos sprites)

## Probar en PC
1) Ejecuta `create_descontrolzombies.py` desde la raíz del repositorio.
2) Abre `DescontrolZombies/project.godot` con Godot 4.3.
3) Ejecuta (Play).

## APK debug por GitHub Actions
1) Sube a GitHub (push a main).
2) Ve a Actions -> Generar APK Debug (DescontrolZombies).
3) Descarga Artifact: DescontrolZombies-debug-apk.
""")

print("Listo. Se creó DescontrolZombies/ y el workflow de GitHub Actions.")
print("Ahora haz commit y push.")
